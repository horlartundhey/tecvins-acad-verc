...
const Hero = () => {
  ...
  useEffect(() => {
    ...
    const resizeCanvas = () => {
      ...
      initSpheres(); // Reinitialize spheres when resizing
    };

    // Initialize the gradient spheres
    const initSpheres = () => {
      if (!canvas) return;

      spheres = [];
      // Create larger gradient spheres with movement parameters
      ...
      // Additional spheres for visual interest
      spheres.push({
        x: canvas.width * 0.5,
        y: canvas.height * 0.3,
        radius: 150,
    ...
    const drawSpheres = () => {
      ...
      spheres.forEach((sphere) => {
        // Update sphere position with smooth wave-like motion
        ...
        const pulseAmount = Math.sin(time * 1.5 + sphere.phase) * 10;
        const currentRadius = sphere.radius + pulseAmount;
        
        // Create and update radial gradient
        const gradient = ctx.createRadialGradient(sphere.x, sphere.y, 0, sphere.x, sphere.y, currentRadius);
        
        // Adjust color opacity slightly based on time for a subtle breathing effect
        ...
        gradient.addColorStop(1, `${sphere.color2}00`); // Fade to transparent
  ...
  // Set images loaded after a short delay to ensure animations start properly
  useEffect(() => {
    ...
    <section className="bg-[#FAFAFA] py-16 pb-32 relative overflow-hidden min-h-[800px]">
      ...
      <div className="container mx-auto px-4 relative" style={{ zIndex: 2 }}>
        ...
        <div className="max-w-4xl mx-auto text-center mb-16">
          ...
          <div className="mt-8">
            <Button className="bg-[#3B9790] hover:bg-teal-600 text-white px-8 py-5 rounded-xl text-lg font-medium shadow-md transition duration-300 flex items-center">
              Begin your learning journey
              <HiOutlineRocketLaunch className="ml-2 h-8 w-8" />
            </Button>
          </div>
        </div>
      </div>
      ...
      <div className="relative mt-20 mb-8">
        {/* Updated more stylish curved line that extends edge to edge */}
        <div className="absolute inset-x-0 w-full" style={{ zIndex: 1 }}>
          <div className="relative w-full left-0 right-0">
            ...
            >
              {/* Updated path with more natural curves */}
              <path 
                d="M0,60 C250,-40 350,160 600,60 C850,-40 950,160 1200,60" 
                stroke="#F87171" 
                strokeWidth="4" 
                fill="none" 
                strokeLinecap="round"
        ...
        {/* Images container with proper spacing to align with the curved line */}
        <div className="container mx-auto px-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 lg:gap-8 relative z-10">
            {/* Image 1 - Student */}
            <div
              className={`w-full aspect-square bg-red-200 rounded-xl overflow-hidden transform transition-all duration-3000 ease-in-out ${
                imagesLoaded ? "translate-y-0" : "translate-y-8"
              }`}
              style={{
            ...
            {/* Image 2 - Coding */}
            <div
              className={`w-full aspect-square bg-purple-300 rounded-xl overflow-hidden transform transition-all duration-3000 ease-in-out ${
                imagesLoaded ? "translate-y-0" : "translate-y-8"
              }`}
              style={{
            ...
            {/* Image 3 - Designer */}
            <div
              className={`w-full aspect-square bg-orange-200 rounded-xl overflow-hidden transform transition-all duration-3000 ease-in-out ${
                imagesLoaded ? "translate-y-0" : "translate-y-8"
              }`}
              style={{
                animation: imagesLoaded ? "float-up 4s ease-in-out infinite" : "none",
            ...
            {/* Image 4 - VR/AR */}
            <div
              className={`w-full aspect-square bg-teal-200 rounded-xl overflow-hidden transform transition-all duration-3000 ease-in-out ${
                imagesLoaded ? "translate-y-0" : "translate-y-8"
              }`}
              style={{
                animation: imagesLoaded ? "float-down 4s ease-in-out infinite" : "none",
